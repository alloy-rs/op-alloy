...
use super::{BlockInfo, L1BlockInfoBedrock, L1BlockInfoEcotone, L1BlockInfoTx, L2BlockInfo};
...
/// The error for execution payload
#[derive(Debug)]
pub enum ExecutionPayloadError {
    /// invalid genesis hash in the execution payload.
    InvalidGenesisHash,
    /// Missing L1 deposit transaction in an L2 block.
    MissingL1InfoDeposit(B256),
    ///  Unexpected transaction type was encountered.
...
impl core::fmt::Display for ExecutionPayloadError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ExecutionPayloadError::InvalidGenesisHash => {
                write!(f, "Invalid genesis hash for the payload")
            }
            ExecutionPayloadError::MissingL1InfoDeposit(hash) => {
                write!(f, "Missing L1 information deposit transaction for block hash: {:?}", hash)
            }
            ExecutionPayloadError::UnexpectedTransactionType(ty) => {
                write!(f, "Unexpected transaction type: {:?}", ty)
            }
            ExecutionPayloadError::DecodingError(e) => write!(f, "Error decoding data: {}", e),
            ExecutionPayloadError::MissingSystemConfig => {
                write!(f, "System configuration missing in genesis block")
            }
        }
    }
}
#[cfg(not(feature = "std"))]
impl std::error::Error for ExecutionPayloadError {}

type Result<T> = core::result::Result<T, ExecutionPayloadError>;
        ...
        let (l1_origin, sequence_number) =
            if execution_payload.block_number == rollup_config.genesis.l2.number {
                if execution_payload.block_hash != rollup_config.genesis.l2.hash {
                    return Err(ExecutionPayloadError::InvalidGenesisHash);
                }
                (rollup_config.genesis.l1, 0)
            } else {
                if execution_payload.transactions.is_empty() {
                    return Err(ExecutionPayloadError::MissingL1InfoDeposit(
                ...
                if ty != OpTxType::Deposit as u8 {
                    return Err(ExecutionPayloadError::UnexpectedTransactionType(format!(
                        "Expected Deposit, found {:?}",
                        ty
                    )));
                }
                let tx = OpTxEnvelope::decode_2718(&mut execution_payload.transactions[0].as_ref())
                    .map_err(|e| ExecutionPayloadError::DecodingError(e.to_string()))?;
                ...
                match tx {
                    ...
                    _ => {
                        return Err(ExecutionPayloadError::UnexpectedTransactionType(format!(
                            "Unexpected payload transaction type: {:?}",
                            tx.tx_type()
                        )))
                    }
                }
    ...
    pub fn to_system_config(&self, rollup_config: &RollupConfig) -> Result<SystemConfig> {
        ...
        if execution_payload.block_number == rollup_config.genesis.l2.number {
            ...
            return rollup_config
                ...
                .system_config
                .clone()
                .ok_or(ExecutionPayloadError::MissingSystemConfig);
        }

        if execution_payload.transactions.is_empty() {
            return Err(ExecutionPayloadError::MissingL1InfoDeposit(execution_payload.block_hash));
        }
        let ty = execution_payload.transactions[0][0];
        if ty != OpTxType::Deposit as u8 {
            return Err(ExecutionPayloadError::UnexpectedTransactionType(format!(
        ...
        Self {
            execution_payload: L2ExecutionPayload {
                ...
                transactions: body
                    .into_iter()
                    .map(|tx| {
                        let mut buf = Vec::with_capacity(tx.inner_length());
